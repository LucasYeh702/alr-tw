...
ALR-TW v0.9.0 is the agent-neutral public preview of the Taiwan-law research safety harness. An external agent or LLM may create and advance a research run over MCP and propose issues or authority locators, while source acquisition, research obligations, evidence promotion, answer validation, retention, and purge remain server-owned. The model is civil-law oriented: statutory text and legal time come first; ordinary judgments are classified by court and section role; Constitutional Court majority reasoning is kept separate from individual opinions.
...
> v0.9.0 remains a public preview. A qualified professional must still verify every answer against official text, the applicable legal time, and the facts of the matter.

> The current `main` working tree is v0.9.0; it does not claim complete
...
The v0.9.0 surface provides legacy `hlExportPDF` and
...
The v0.9.0 surface also includes a provider-neutral applicability resolver for
...
The current v0.9.0 contracts also provide an optional semantic
...
In v0.9.0 the bundled `ResearchService` does not inject or persist live-provider
