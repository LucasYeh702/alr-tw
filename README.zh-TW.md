...
receipt 完整且其餘閘門均通過時，內建路徑才可能回 `ordinary`；缺 receipt 最高為
`conditional`，不一致則 fail closed。Finalization 只授權進入起草
（`safe_to_draft`），不授權呈現答案；只有 `validate_legal_answer` 的
`validated`／`qualified` 結果可展示。receipt 不代表全域召回、裁判確定或實務一致。
...
裁判檢索可用 `/quick 問題`、`快速模式：問題`，或 structured
`constraints.research_depth=quick` 啟用。搭配 `execute_legal_research` 時，server
會在一次 MCP 呼叫內完成 privacy screen、候選召回、最多五件 JID／正式字號與
官方全文回查，以及 evidence sufficiency。`hybrid_verified` quick 先用 TLR／相容
provider 召回；只有無候選或候選 provider 失敗才退回司法院關鍵詞搜尋。

Quick mode 只縮減 breadth：預設不展開 counter-authority、lineage 或未明示的
法規研究；official verification 與最終 `validate_legal_answer` 不可跳過。類案
quick 本身固定揭露 bounded top-K，因此即使全部候選驗證成功，仍只允許
`qualified`／`conditional`；未通過或被截斷候選再加限制，`0` 件成功仍是
`refusal_only`。

未完成或被擋下的 run 可從 `get_legal_research_state.research_brief` 取得已驗證來源
locator、義務進度、blocker 與安全下一步。此 brief 不含草稿結論，固定
`answer_authorized=false`、`safe_to_present=false`，不可當作答案。
...
若強制終止本機 stdio process 後宿主仍顯示舊的 connected 狀態，先停用再啟用 MCP
設定或重啟宿主；仍無法連線時移除後依原設定重新加入。宿主殘留狀態不是 runtime
健康證據。
