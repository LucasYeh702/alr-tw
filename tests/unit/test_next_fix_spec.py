...
def test_g5_guard_size_caps_stay_aligned():
...
def test_g5_over_cap_text_files_are_not_silently_skipped(tmp_path: Path):
    forbidden_checker = _load_forbidden_checker_module()
...
def test_g6_both_guards_reject_utf16_le_forbidden_markers(tmp_path: Path):
...
def test_g7_both_guards_reject_cjk_adjacent_taiwan_identity_numbers(tmp_path: Path):
    synthetic_sensitive_id = "A" + "123456789"
    candidate = tmp_path / "fixture.md"
...
def test_g7_both_guards_reject_cjk_adjacent_judgment_identifiers(tmp_path: Path):
    real_shaped_jid = ",".join(["TPDV", "112", "訴", "123", "20240101", "1"])
    candidate = tmp_path / "fixture.md"
...
def test_g7_both_guards_reject_non_conjunctive_synthetic_namespace(tmp_path: Path):
...
def test_g7_current_public_tree_has_no_domain_guard_false_positives():
    forbidden_checker = _load_forbidden_checker_module()
