...
- `alr-tw.research-execution/v1`：單次 autonomous runner 的 stop reason、step count、
  overall/per-step `elapsed_ms`、精簡 provider calls、目前 state 及 finalization；
- `alr-tw.research-evidence-bundle/v1`：到達 draft boundary 後最多五個新鮮、
  evidence-eligible sources 與每來源最多八個 exact spans；固定
  `global_absence_claim_allowed=false`、`answer_authorized=false`；
