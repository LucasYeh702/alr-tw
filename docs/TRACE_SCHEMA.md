...
## v0.9.0 research state records

v0.9.0 流程以持久化 `ResearchRun` 與逐步 result 取代 caller 組裝 authority trace：
...
舊 `alr-tw.agentic_trace/v1` 保留 synthetic／externally-driven compatibility，但不得用它把 caller-attested source 注入 v0.9.0 evidence。
