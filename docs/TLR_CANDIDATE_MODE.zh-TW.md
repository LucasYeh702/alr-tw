...
在 Python library（函式庫）層級，`identifier_resolution` 屬於 adapter/verifier（資料轉接器/驗證器）的信任邊界，必須只由 deployer（部署者）的 resolver layer（解析層）設定；手動填入即代表為該紀錄背書。

可執行 quickstart（快速入門）：
[`examples/identifier_backed_demo.py`](../examples/identifier_backed_demo.py)
使用 synthetic records（合成資料）示範從 TLR candidate（候選資料）到 identifier-backed
`verified_cache`（識別碼支撐的驗證快取）的流程，包含 opt-in（選擇性啟用）hash match（雜湊相符）路徑，以及 hash mismatch（雜湊不符）與 default-disabled（預設關閉）的 fail-closed（保守拒答）路徑。
