...
Legacy verified-cache helpers may check field presence, but field presence is not byte verification. A deployment promotion pipeline must resolve the official original and recompute bytes/hash as defined in [ARCHITECTURE_CONTRACT.md](ARCHITECTURE_CONTRACT.md); the v0.8.0 MCP boundary instead accepts only server-owned provider/resolver results. `ready_for_draft` is not sufficiency, and counter-authority scoped misses cannot prove global absence or consensus.
