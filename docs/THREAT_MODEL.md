...
Legacy verified-cache helpers may check field presence, but field presence is not byte verification. In v0.9.1, raw mappings entering the legacy answer/provider-promotion boundary are caller-controlled, caller-declared resolver status is discarded, and metadata-only answer validation cannot authorize presentation. A deployment promotion pipeline must resolve the official original and recompute bytes/hash as defined in [ARCHITECTURE_CONTRACT.md](ARCHITECTURE_CONTRACT.md); the MCP boundary accepts only server-owned provider/resolver results. `ready_for_draft` is not sufficiency, and counter-authority scoped misses cannot prove global absence or consensus.
