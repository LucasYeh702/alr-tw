...
本文件定義 v0.10.0 的前端無關接口。任何能呼叫 MCP tool 的法律
...
The v0.10.0 public surface also includes structural applicability resolution,
...
The v0.10.0 surface adds an optional semantic-verifier sidecar. It can
...
The v0.10.0 provider boundary also exposes a common conformance validator and an
...
Its additive `active_mcp_tool_profile` and `available_mcp_tool_names` fields are
derived from the same session-fixed catalog used by `tools/list` and
`tools/call`. Clients must inspect this response instead of assuming that
historical law, counter-authority search, or a particular material type is
available. These fields describe routing only; they do not create evidence or
transfer trust ownership.
...
This is the v0.10.0-compatible default. ALR-TW plans and executes its bounded
