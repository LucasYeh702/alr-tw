...
ALR-TW v0.8.0 將「agent 決定如何推理」與「server 決定何者可信」分開。外部 agent 可以提出查詢、逐步呼叫工具、提交爭點與法源 locator 並起草答案，但不能注入正式證據或跳過 obligations。v0.8.0 將這條邊界做成 agent-neutral interoperability contract，不依賴特定前端專案。
...
`alr_tw.*` 是 v0.8.0 中立 contracts、providers、research 與 storage 的主命名空間。`tw_legal_rag_mcp.*` 保留 legacy synthetic／trace 工具並承載 MCP stdio server。兩者共用 source tier 與 fail-closed invariants；新功能不得反向依賴 client-controlled provenance。
...
runtime 保證。v0.8.0 內建 `ResearchService` 尚未注入或持久化 live-provider
